...
> **Reference:** `tailwindcss.com/docs` — every utility class listed with live examples, searchable by property name.
...
The core idea — every class does **one specific thing**. Padding, flex, colors, hover states — there's a class for each. These utility classes are composed together to build any UI without writing custom CSS.
...
| Unused styles | Easy to accumulate dead code | Unused styles purged automatically in production |
...
| Custom styles | Full freedom | Still fully possible, works within a design system |
